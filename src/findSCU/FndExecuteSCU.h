...
public: 
	...
	*  (class CFndExecuteSCU) to store a pointer to the current association
	...
	*  (class CFndExecuteSCU) to store the current presentation context ID
	*  before the callback object is used.
...
*/
class CFndExecuteSCUDefaultCallback: public CFndExecuteSCUCallback
{
public:
	/** constructor
...
public:
	...
	*/  
	OFCondition performQuery(
		...
		CFndExecuteSCUCallback *callback = NULL,
	...
	*  @return EC_Normal if successful, an error code otherwise
	*/  
	OFCondition performQuerybyDataset(
		const char *peer,
		unsigned int port,
		const char *ourTitle,
		const char *peerTitle,
...
private:
	...
	*/   
	OFCondition findSCU(
		...
		int cancelAfterNResponses,
		OFList<OFString> *overrideKeys,
		CFndExecuteSCUCallback *callback = NULL) const;

	// dataset
	/** perform a single C-FIND transaction on association that is already open.
	*  @param assoc DIMSE association
	*  @param dataset for the query file to be used with this query. May be NULL.
	*  @param repeatCount number of times this query should be repeated
	*    (for debugging purposes, works only with default callback)
	*  @param abstractSyntax SOP Class UID or Meta SOP Class UID of service
	*  @param blockMode DIMSE blocking mode
